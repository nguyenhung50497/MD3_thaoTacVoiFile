...
let server = http.createServer(function (req, res) {
    ...
    function displayProduct(arr) {
        ...
        for (let i = 0; i < arr.length; i++) {
            ...
            str += `<td><button class="btn btn-danger" id="deleteProduct` + i + `" onclick="deleteProduct(` + i + `)">Delete</button></td>`
            str += `<td><button class="btn btn-primary" id="updateProduct` + i + `" onclick="updateProduct(` + i + `)">Update</button></td>`
            str += '</tr>';
        };
