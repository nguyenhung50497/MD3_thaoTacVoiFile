...
let server = http.createServer(function (req, res) {
    let dataFile = [];
    let list = JSON.parse(fs.readFileSync('./data/data.json','utf8'));
    // if (list.length > 0) {
    ...
    if (methodRequest === 'GET') {
        ...
        req.on('end', () => {
            ...
            list.push(newData);
            let json = JSON.stringify(listen);
            ...
            str += `<td><button class="btn btn-danger" onclick="deleteProduct(` + list.length + `)">Delete</button></td>`
            str += `<td><button class="btn btn-primary" onclick="updateProduct(` + list.length + `)">Update</button></td>`
            str += '</tr>';
